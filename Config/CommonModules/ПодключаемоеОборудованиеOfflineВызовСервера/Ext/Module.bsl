...
Функция СоздатьИЗаполнитьОтчетОПродажах(ПараметрыУстройства, ТаблицыДанных, Комментарий = "", ЭтоПатент)
	...
	СтруктураПараметровДокумента.Вставить("ДеятельностьНаПатенте", ЭтоПатент 
		И УчетнаяПолитика.ПрименяетсяУСНПатент(ПараметрыУстройства.Организация, ТаблицыДанных.ДатаЗакрытияСмены));
	...
	ЗаполнениеДокументов.Заполнить(ОтчетОРозничныхПродажахОбъект, СтруктураПараметровДокумента);
