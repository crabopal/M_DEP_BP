...
Функция СведенияОсновногоДоверителя(Доверенность) Экспорт
	...
	Если РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.РосОргДовер") <> Неопределено Тогда
		
		СвРосОрг = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит.РосОргДовер.СвРосОрг;
		
		СведенияДоверителя.ИНН = СвРосОрг.ИННЮЛ;
		СведенияДоверителя.КПП = СвРосОрг.КПП;
		СведенияДоверителя.КПП = СвРосОрг.ОГРН;
		СведенияДоверителя.Наименование = СвРосОрг.НаимОрг;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.ИнОргДовер") <> Неопределено Тогда
		
		СвИнОрг = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит.ИнОргДовер.СвИнОрг;
		
		СведенияДоверителя.ИНН = СвИнОрг.ИННЮЛ;
		СведенияДоверителя.КПП = СвИнОрг.КПП;
		СведенияДоверителя.Наименование = СвИнОрг.НаимИО;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.ИПДовер") <> Неопределено Тогда
		
		ИПДовер = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит.ИПДовер;
		
		СведенияДоверителя.ИНН = ИПДовер.ИННФЛ;
		СведенияДоверителя.ОГРН = ИПДовер.ОГРНИП;
		СведенияДоверителя.Наименование = ИПДовер.НаимИП;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.Доверит.РосОргДовер") <> Неопределено Тогда
		
		СвРосОрг = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит[0].Доверит.РосОргДовер.СвРосОрг;
		
		СведенияДоверителя.ИНН = СвРосОрг.ИННЮЛ;
		СведенияДоверителя.КПП = СвРосОрг.КПП;
		СведенияДоверителя.ОГРН = СвРосОрг.ОГРН;
		СведенияДоверителя.Наименование = СвРосОрг.НаимОрг;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.Доверит.ИнОргДовер") <> Неопределено Тогда
		
		СвИнОрг = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит[0].Доверит.ИнОргДовер.СвИнОрг;
	...
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Довер.СвДоверит.Доверит.ИПДовер") <> Неопределено Тогда
		
		ИПДовер = Результат.ДанныеДоверенности.Документ.Довер.СвДоверит[0].Доверит.ИПДовер;
		
		СведенияДоверителя.ИНН = ИПДовер.ИННФЛ;
		СведенияДоверителя.ОГРН = ИПДовер.ОГРНИП;
		Если ЗначениеЗаполнено(ИПДовер.НаимИП) Тогда
			СведенияДоверителя.Наименование = ИПДовер.НаимИП;
		Иначе
			СведенияДоверителя.Наименование = СтрШаблон("%1 %2 %3", ИПДовер.СведФЛ.ФИО.Фамилия, ИПДовер.СведФЛ.ФИО.Имя, ИПДовер.СведФЛ.ФИО.Отчество);
		КонецЕсли;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.РосОргДовер") <> Неопределено Тогда
		
		РосОргДовер = Результат.ДанныеДоверенности.Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.РосОргДовер;
		
		СведенияДоверителя.ИНН = РосОргДовер.ИННЮЛ;
		СведенияДоверителя.КПП = РосОргДовер.КПП;
		СведенияДоверителя.ОГРН = РосОргДовер.ОГРН;
		СведенияДоверителя.Наименование = РосОргДовер.НаимОрг;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.ИнОргДовер") <> Неопределено Тогда
		
		ИнОргДовер = Результат.ДанныеДоверенности.Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.ИнОргДовер;
		
		СведенияДоверителя.ИНН = ИнОргДовер.ИННЮЛ;
		СведенияДоверителя.КПП = ИнОргДовер.КПП;
		СведенияДоверителя.Наименование = ИнОргДовер.НаимИО;
		
	ИначеЕсли РаботаСФайламиБЭД.ЗначениеСвойстваXDTO(
		Результат.ДанныеДоверенности, "Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.ИПДовер") <> Неопределено Тогда
		
		ИПДовер = Результат.ДанныеДоверенности.Документ.Передов.СвДовПер.СвОснДовер.СвДовер0.ИПДовер;
