...
Функция ОбработкаРезультатаПолученияДанныхАбонентов(Знач ПолученныеДанные, Отказ)
	...
	Для Каждого Абонент Из ПолученныеДанные.identifierInfos Цикл
		...
		Если Абонент.Свойство("abonentDocTypes") Тогда
			НоваяСтрока.КодыПоддерживаемыхТиповДокументов = ?(ЗначениеЗаполнено(Абонент.abonentDocTypes),
				ОбщегоНазначения.ЗначениеВJSON(Абонент.abonentDocTypes), "");
		КонецЕсли;
...
// * КодыПоддерживаемыхТиповДокументов - Строка
Функция НоваяТаблицаАбонентов()
	...
	Таблица.Колонки.Добавить("КодыПоддерживаемыхТиповДокументов", Типы().Строка);
