...
Функция ТекстЗапросаОтражениеДОППВДекларацииПоКосвеннымНалогам()
	...
	|	МАКСИМУМ(РезультирующаяТаблица.ДокументОприходования) КАК ДокументОприходования,
	|	МАКСИМУМ(РезультирующаяТаблица.СтатусВвозаЧисло) КАК СтатусВвозаЧисло
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	АкцизыПоКБК.ДОПП КАК ДОПП,
	|	АкцизыПоКБК.КБК КАК КБК,
	|	ВЫРАЗИТЬ(АкцизыПоКБК.ОбеспечительныйПлатеж КАК ЧИСЛО(15, 0)) КАК ОбеспечительныйПлатежВсего,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0)) КАК РанееОтраженнаяСумма,
	|	ВЫРАЗИТЬ(АкцизыПоКБК.ОбеспечительныйПлатеж - ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0)) КАК ОбеспечительныйПлатежКОтражению
	|ПОМЕСТИТЬ ОбеспечительныеПлатежиКОтражению
	|ИЗ
	|	АкцизыПоКБК КАК АкцизыПоКБК
	|		ЛЕВОЕ СОЕДИНЕНИЕ РанееОтраженныеВОтчетностиДОПП КАК РанееОтраженныеВОтчетностиДОПП
	|		ПО (РанееОтраженныеВОтчетностиДОПП.ДОПП = АкцизыПоКБК.ДОПП)
	|			И (РанееОтраженныеВОтчетностиДОПП.КБК = АкцизыПоКБК.КБК)
	|ГДЕ
	|	(АкцизыПоКБК.ОбеспечительныйПлатеж = 0
	|			ИЛИ (ВЫРАЗИТЬ(АкцизыПоКБК.ОбеспечительныйПлатеж - ЕСТЬNULL(РанееОтраженныеВОтчетностиДОПП.СуммаНалогаАкциза, 0) КАК ЧИСЛО(15, 0))) > 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДОПП
	|;
