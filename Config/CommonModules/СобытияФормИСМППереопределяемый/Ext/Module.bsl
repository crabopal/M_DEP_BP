...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	Если ИнтеграцияИСМПБП.ЕстьДобавленныеКолонкиИСМП(Форма.Объект.Товары) Тогда
		Для Каждого СтрокаТаблицы Из Форма.Объект.Товары Цикл
			ОписаниеНоменклатуры 						= ОбщегоНазначенияИС.ОписаниеНоменклатуры(СтрокаТаблицы.Номенклатура);
			ВидПродукцииИС 								= ОписаниеНоменклатуры[СтрокаТаблицы.Номенклатура].КлючВидовПродукции;
			СтрокаТаблицы.МаркируемаяПродукция 			= ЗначениеЗаполнено(ВидПродукцииИС);
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТаблицы, "ВидПродукцииИС") Тогда
				СтрокаТаблицы.ВидПродукцииИС 			= ВидПродукцииИС;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
