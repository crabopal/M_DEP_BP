Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт 
	...
	КомандаПечати.Представление = "ИНВ-1 (инвентаризационная опись)";
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
	КомандаПечати.СписокФорм    = "ФормаСписка,ФормаДокументаУпр";
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ18";
	КомандаПечати.Представление = "ИНВ-18 (сличительная ведомость)";
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
	КомандаПечати.СписокФорм    = "ФормаСписка,ФормаДокументаУпр";
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ИНВ22";
	КомандаПечати.Представление = "ИНВ-22 (приказ)";
	КомандаПечати.Обработчик    = "УправлениеПечатьюБПКлиент.ВыполнитьКомандуПечати";
	КомандаПечати.СписокФорм    = "ФормаСписка,ФормаДокументаУпр";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт 	
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИНВ1") Тогда
	 	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ИНВ1", "ИНВ1", 
			ПечатьИНВ1(МассивОбъектов),"Документ.ИнвентаризацияОСМДЭП.ИНВ1");
	КонецЕсли;
	
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИНВ18") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ИНВ18", "ИНВ18", 
		ПечатьИНВ18(МассивОбъектов),"Документ.ИнвентаризацияОСМДЭП.ИНВ18");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИНВ22") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ИНВ22", "ИНВ22", 
		ПечатьИНВ22(МассивОбъектов),"Документ.ИнвентаризацияОСМДЭП.ИНВ22");
	КонецЕсли;
...
////  Неопределено.
////
Функция ПечатьИНВ1(МассивОбъектов) Экспорт
	...
	|	ИнвентаризацияОС.МОЛ
	|ИЗ
	|	Документ.ИнвентаризацияОСМДЭП КАК ИнвентаризацияОС
	|ГДЕ
	...
	|ИЗ
	|	Документ.ИнвентаризацияОСМДЭП.ИнвентаризационнаяКомиссия КАК ИнвентаризацияОСИнвентаризационнаяКомиссия
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ПервоначальныеСведенияОСБухгалтерскийУчетСрезПоследних.ИнвентарныйНомер = ИСТИНА
	|			ТОГДА ПервоначальныеСведенияОСБухгалтерскийУчетСрезПоследних.ИнвентарныйНомер 
	|		ИНАЧЕ ИнвентаризацияОСОС.ОсновноеСредство.Код
	...
	//processor
	|ИЗ
	|	Документ.ИнвентаризацияОСМДЭП.ОС КАК ИнвентаризацияОСОС
	...
	ДанныеМОЛ = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛиц(Док.Организация, СписокМОЛ, Док.Дата);
	...
	Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
		...
		Если НЕ ОбщегоНазначения.ПроверитьВыводТабличногоДокумента(ТабДокумент, ПодписьСПримечанием) Тогда
		...
		Подпись.Параметры.ЗаголовокРазделаПодписей = ?(ВыводитьЗаголовок, 
													   ...
													   "");
		Подпись.Параметры.Должность                = ПолучитьДолжностьФизЛица(СтрокаМОЛ.ФизическоеЛицо);
	...
	Если НЕ (ПредседательКомиссии = Неопределено) Тогда
		...
		Подпись.Параметры.Должность                = ПолучитьДолжностьФизЛица(ПредседательКомиссии.ФизЛицо); //ДанныеПредседателя.Должность;
	...
	Для Каждого ЧленКомиссии Из ТаблицаИнвентаризационнаяКомиссия Цикл
		...
		Подпись.Параметры.ЗаголовокРазделаПодписей = ?(ВыводитьЗаголовок, 
													   ...
													   "");
		Подпись.Параметры.Должность                = ПолучитьДолжностьФизЛица(ЧленКомиссии.ФизЛицо);
	...
	Для Каждого СтрокаМОЛ Из ДанныеМОЛ Цикл
		...
		Если НЕ ОбщегоНазначения.ПроверитьВыводТабличногоДокумента(ТабДокумент, ПодписьСДатой) Тогда
	...
	ОтветственныеЛица = ОтветственныеЛицаБП.ОтветственныеЛица(Док.Организация, Док.Дата);
		
	Подпись.Параметры.Должность                = "Главный бухгалтер";
	Подпись.Параметры.РасшифровкаПодписи       = ОтветственныеЛица.ГлавныйБухгалтерПредставление;
...
////
Функция ПечатьИНВ18(МассивОбъектов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ИнвентаризацияОС.ПодразделениеОрганизации.Наименование ПОДОБНО """"
	|			ТОГДА ИнвентаризацияОС.ПодразделениеОрганизации.Наименование
	|		ИНАЧЕ ВЫРАЗИТЬ(ИнвентаризацияОС.ПодразделениеОрганизации.Наименование КАК СТРОКА(200))
	...
	// Выведем подписи МОЛов
	ДанныеМОЛ = ОбщегоНазначенияБПВызовСервера.ДанныеФизЛиц(ССылка.Организация, СписокМОЛ, ССылка.Дата);
...
////
Функция ПечатьИНВ22(МассивОбъектов) Экспорт
