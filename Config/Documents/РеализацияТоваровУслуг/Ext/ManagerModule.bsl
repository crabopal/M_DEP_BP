...
Функция СоздатьДокументИзСтруктуры(ДанныеСтруктуры, ТекстПрогресса = "") Экспорт
	...
	Если ЗначениеЗаполнено(НомерУведомления) И РеквизитыСФ <> Неопределено И ЗначениеЗаполнено(РеквизитыСФ.СчетФактура) Тогда
		СчетФактураОбъект = РеквизитыСФ.СчетФактура.ПолучитьОбъект();
		СчетФактураОбъект.ОтредактированНомерСчетаФактуры = Истина;
		СчетФактураОбъект.ПредставлениеНомера = НомерУведомления;
		СчетФактураОбъект.НомерВходящегоДокумента = НомерУведомления;
		СчетФактураОбъект.Записать();
	КонецЕсли;
