...
Функция СформироватьОтчетНаСервере(Знач ГраницыПересчетаЛистов = Неопределено)
	...
	ПараметрыОтчета = Отчеты.КассоваяКнигаОперационная.ПустыеПараметрыКомпоновкиОтчета();
	Отчеты.КассоваяКнигаОперационная.ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, ЭтотОбъект);
	
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		
		АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
		Отчеты.КассоваяКнигаОперационная.СформироватьОтчет(ПараметрыОтчета, АдресХранилища);
		
	Иначе
		
		РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор,
			"Отчеты.КассоваяКнигаОперационная.СформироватьОтчет",
			ПараметрыОтчета,
...
Процедура ПоказатьТабличныйДокумент(ТабДокумент)
	...
	Результат.Очистить();
	Результат.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КассоваяКнигаОперационная";
...
Функция ПолучитьДополнительныеПараметры(Знач ПараметрыОтчета)
	...
	ОтветственныеЛица = Отчеты.КассоваяКнигаОперационная.ПолучитьОтветственныхИУполномоченных(ПараметрыОтчета);
	ЗаполнитьЗначенияСвойств(Результат, ОтветственныеЛица);
	Результат.НомерЛистаНачало = Отчеты.КассоваяКнигаОперационная.ПолучитьНомерЛистаНачало(ПараметрыОтчета);
...
Процедура ИзменениеРеквизитаШапкиОтчета(Форма)
	...
	ПараметрыОтчета.Вставить("ПодразделениеОрганизации", Отчет.ПодразделениеОрганизации);
	ПараметрыОтчета.Вставить("ПлатежныйАгент",           Отчет.ПлатежныйАгент);
	ПараметрыОтчета.Вставить("ПассажирскиеПеревозки",    Форма.ПассажирскиеПеревозки); //ЮЦАУ
