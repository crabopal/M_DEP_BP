...
Процедура ПолучитьСведенияОПоказателяхОтчета()
	...
	Если ДокументооборотСКОПовтИсп.ЭтоРежимОблачной1СО() И Не ДокументооборотСКОПовтИсп.ЭтоБизКуб() Тогда
		...
		ЕстьПоддержкаЗаполнения = РелизованаПоддержкаЗаполненияВОблачнойОтчетности();
		ЕстьПоддержкаРасшифровки = РелизованаПоддержкаРасшифровкиВОблачнойОтчетности();
		Если ЕстьПоддержкаЗаполнения Или ЕстьПоддержкаРасшифровки Тогда
			РегламентированнаяОтчетность.ВставитьПоказательВСтруктуру(АвтозаполняемыеПоказатели, 
...
Процедура ЗаполнитьАвто(Команда) Экспорт
	Отказ = Ложь;
	РегламентированнаяОтчетностьКлиент.ПредложитьОформитьЗаявлениеНаПодключениеПриЗаполнении(ЭтотОбъект, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
...
//   Булево 
&НаКлиентеНаСервереБезКонтекста
Функция РелизованаПоддержкаЗаполненияВОблачнойОтчетности() Экспорт
	Возврат Истина;
КонецФункции
...
//   Булево 
&НаКлиентеНаСервереБезКонтекста
Функция РелизованаПоддержкаРасшифровкиВОблачнойОтчетности() Экспорт
	Возврат Ложь;
КонецФункции

&НаКлиенте
Процедура ПослеВыборИсточникаДанныхДляОблака() Экспорт
...
&НаКлиенте
Процедура ПослеПолученияДанныхДляОблака() Экспорт
