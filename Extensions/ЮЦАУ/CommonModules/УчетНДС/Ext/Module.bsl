...
&ИзменениеИКонтроль("ПодготовитьДанныеШапкиСчетаФактуры1137")
Функция Расш1_ПодготовитьДанныеШапкиСчетаФактуры1137(ВыборкаСФ, СтрокаТаблицыРеквизиты, ПлатежноРасчетныеДокументы, ДополнитьДаннымиУПД, ФормированиеЭД)
	...
	#Вставка
	ДанныеШапки.Вставить("Комментарий",     Реквизиты.ДокументОснование.Комментарий);  //ЮЦАУ	
	#КонецВставки
	...
	Если ВыборкаСФ.ЭтоСчетФактураВыданный Тогда
		...
		КонецЕсли;
	Иначе
		Если СтрДлина(СокрЛП(Реквизиты.ИННпоставщика)) = 12 Тогда
			ДанныеШапки.Вставить("Свидетельство", БухгалтерскийУчетПереопределяемый.СвидетельствоОРегистрации(
			СведенияОПоставщике,
			ВыборкаСФ.Дата));
		КонецЕсли;
	...
	Если ДополнитьДаннымиУПД Тогда
		...
		Если Реквизиты.Свойство("ПередачаДляСобственныхНуждИСМП") Тогда
			ДанныеШапки.Вставить("ПередачаДляСобственныхНуждИСМП", Реквизиты.ПередачаДляСобственныхНуждИСМП);
		КонецЕсли;
