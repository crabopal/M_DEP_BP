...
&Вместо("ПустыеПараметрыКомпоновкиОтчета")
Функция Расш1_ПустыеПараметрыКомпоновкиОтчета()
	...
	ПараметрыОтчета.Вставить("ПассажирскиеПеревозки",  Ложь); //ЮЦАУ
...
&Вместо("ЗаполнитьПараметрыИзФормы")
Процедура Расш1_ЗаполнитьПараметрыИзФормы(ПараметрыОтчета, Форма)
	...
	ПараметрыОтчета.ПассажирскиеПеревозки      = Форма.ПассажирскиеПеревозки;//ЮЦАУ
	
КонецПроцедуры

&Вместо("ПодготовитьОтчет")
Функция Расш1_ПодготовитьОтчет(ПараметрыОтчета)
	...
	Если ПараметрыОтчета.ПассажирскиеПеревозки  Тогда
		ПредопределенныеСчетаКассы = Новый Массив;
		ПредопределенныеСчетаКассы.Добавить(ПланыСчетов.Хозрасчетный.ПассажирскиеПеревозки);    // 50 .05.  //ЮЦАУ
		СчетаКассы = БухгалтерскийУчет.СформироватьМассивСубсчетов(ПредопределенныеСчетаКассы);
	ИначеЕсли ПараметрыОтчета.ПлатежныйАгент Тогда
		СчетаКассы = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.КассаПлатежногоАгента);
	Иначе
		ПредопределенныеСчетаКассы = Новый Массив;
		ПредопределенныеСчетаКассы.Добавить(ПланыСчетов.Хозрасчетный.КассаОрганизации);    // 50 .01.  //ЮЦАУ
