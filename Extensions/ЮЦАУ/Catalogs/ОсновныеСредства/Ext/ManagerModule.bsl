...
//	|	ОсновныеСредства.Характеристика   //ЮЦАУ
//	|ИЗ
...
//		|////////////////////////////////////////////////////////////////////////////////
//		|ВЫБРАТЬ
//		|	ЕСТЬNULL(ХозрасчетныйОстатки.СуммаОстатокКт,0) КАК СуммаНачисленнойАммортизации,
...
//		|			Счет В
//		|				(ВЫБРАТЬ
//		|					втДанные.СчетНачисленияАмортизации
//		|				ИЗ
...
//		|////////////////////////////////////////////////////////////////////////////////
//		|ВЫБРАТЬ
//		|	ЕСТЬNULL(ХозрасчетныйОстатки.СуммаОстатокДт,0) КАК СтоимостьОС,   //ЮЦАУ
//		|	ХозрасчетныйОстатки.Субконто1
...
//		|////////////////////////////////////////////////////////////////////////////////
//		|ВЫБРАТЬ
//		|	втДанные.СтоимостьДляВычисленияАмортизации,
//		|	втДанные.ОбъемПродукцииРаботДляВычисленияАмортизации,
//		|	втДанные.КоэффициентАмортизации,
//		|	втДанные.КоэффициентУскорения,
//		|	втДанные.Период,
//		|	втДанные.СчетУчета,
//		|	втДанные.СчетНачисленияАмортизации,
//		|	втДанные.СпособНачисленияАмортизации,
//		|	втДанные.ПервоначальнаяСтоимость,
//		|	втДанные.СрокПолезногоИспользования,
//		|	втДанные.Местонахождение,
//		|	втДанные.МОЛ,
//		|	втДанные.МестонахождениеНаименование,
//		|	втДанные.ОсновноеСредство,
//		|	втДанные.ИнвентарныйНомер,
//		|	ЕСТЬNULL(втАмортизация.СуммаНачисленнойАммортизации, 0) КАК СуммаНачисленнойАммортизации,
//		|	ЕСТЬNULL(втСтоимостьОС.СтоимостьОС, 0) КАК СтоимостьОС
//		|ИЗ
//		|	втДанные КАК втДанные
//		|		ЛЕВОЕ СОЕДИНЕНИЕ втАмортизация КАК втАмортизация
//		|		ПО втДанные.ОсновноеСредство = втАмортизация.Субконто1
//		|		ЛЕВОЕ СОЕДИНЕНИЕ втСтоимостьОС КАК втСтоимостьОС
//		|		ПО втДанные.ОсновноеСредство = втСтоимостьОС.Субконто1";
//		
...
//		Запрос.Текст = 
//		"ВЫБРАТЬ
//		|	ОсновныеСредстваКонтактнаяИнформация.Представление КАК МестоНахождениеОС
//		|ИЗ
//		|	Справочник.ОсновныеСредства.КонтактнаяИнформация КАК ОсновныеСредстваКонтактнаяИнформация
//		|ГДЕ
//		|	ОсновныеСредстваКонтактнаяИнформация.Ссылка = &Ссылка
//		|	И ОсновныеСредстваКонтактнаяИнформация.Вид = &Вид";
//		
//		Запрос.УстановитьПараметр("Вид", Справочники.ВидыКонтактнойИнформации.АдресМестонахожденияОсновныеСредства);
//		Запрос.УстановитьПараметр("Ссылка", ВыборкаОбъектов.Ссылка);
//		
//		ВыборкаДетальныеЗаписи = Запрос.Выполнить().Выбрать();
//		
//		Если  ВыборкаДетальныеЗаписи.Следующий() Тогда
//			Шапка.Параметры.МестоНахождениеОС    = ВыборкаДетальныеЗаписи.МестоНахождениеОС;
//		КонецЕсли;
//		//
...
//		Если ТекущиеСведенияОС.Количество() <> 0 Тогда //ЮЦАУ
//			ШапкаРазделов1и2.Параметры.СуммаНачисленнойАммортизации    = ТекущиеСведенияОС.СуммаНачисленнойАммортизации; //ЮЦАУ
//			ШапкаРазделов1и2.Параметры.ОстаточнаяСтоимость    = ТекущиеСведенияОС.СтоимостьОС - ТекущиеСведенияОС.СуммаНачисленнойАммортизации;
//		КонецЕсли;
...
//		КонецЦикла;
//		// ЮЦАУ
//		ШапкаРаздела7_1.Параметры.Характеристика = ВыборкаОбъектов.Характеристика;
//		//
